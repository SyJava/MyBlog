        ...
        <div class="grid-content content bg-purple blog_bg ">
          <div  style="height:3px; margin: 5px 0; border:none; border-top:3px solid  #1eb9e0;"></div>
            ...
            <el-form  :model="comment" :rules="rules" ref="comment" class="demo-ruleForm">
              ...
              <div style="width: 95%;margin: auto;padding-top: 1%;">
                 评论 <el-tag v-if="this.comment.adminComment">已登录</el-tag>  <el-tag type="info" @click="dialogFormVisible = true" style="float: right;">管理员评论</el-tag>
          ...
          <el-dialog  title="管理员登录" :visible.sync="dialogFormVisible" style="width: 50%;margin: auto;" >
            <el-form :model="loginForm" ref="loginForm" >
              ...
              :rules="{ required: true, message: '请输入验证码', trigger: 'blur' }">
                <el-input  placeholder="请输入验证码" v-model="loginForm.code" auto-complete="off" maxlength="5"></el-input>
                <el-image :src="captchaImg" class="captchaImg" @click="getCaptcha"></el-image>
              </el-form-item>
            </el-form>

            <div slot="footer" class="dialog-footer">
              <el-button v-if="this.comment.adminComment"  @click="subLogout">注 销</el-button>
...
export default {
  ...
  methods:{
    ...
    initComment(){
      const _this = this
      this.axios.get('comment/getComments/' + this.blogId).then(resp=>{
        console.log(resp)
        _this.commentData = resp.data.data
      })
    },
    //发表评论
    submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            const _this = this
            this.axios.post('/comment/saveComments',this.comment).then(resp=>{
              console.log(resp)
              if(resp.status == 200){
                this.initComment()
          ...
          if (valid) {
            ...
            this.axios.post('/login',qs.stringify(this.loginForm) ).then(resp=>{
              ...
              if(resp.data.code == 200){
                //user信息储存在sessionStorage中，窗口关闭即自动退出
                _this.comment.adminComment=true
    ...
    getCaptcha() {
      ...
      subLogout(){
        ...
        this.$confirm('此操作将注销登录, 是否继续?', '提示', {
            ...
            this.comment.adminComment=false
            this.user = false
            this.$message({
              message: '注销成功！',
              type: 'success'
