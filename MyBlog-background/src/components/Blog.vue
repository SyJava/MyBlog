<template>
  <div class="wrapper" style="height: 100vh;overflow-x: hidden;">
        ...
        <div v-model="blogData" class="grid-content content bg-purple blog_bg ">
            ...
            <div style="padding: 1%;">
                <el-avatar style="vertical-align:middle" :src="blogData.user.avatar"></el-avatar>
                <span style="margin-left: 5px;">{{blogData.user.username}}</span>
            ...
            <div style="text-align: center;">
                ...
                <el-tag>{{blogData.isOriginal?'原创':'转载'}}</el-tag>
                <el-tag type="success" style="margin-left: 10px;">{{blogData.type.name}}</el-tag>
<!--                <el-tag v-for="(tag,index) in blogData.tblogTags" :key="index" type="info" style="margin-left: 10px;">{{tag.tag.name}}</el-tag>-->
  ...
  methods:{
      initBlog(){
          ...
          this.axios.get('/blog/getByBlogId?id=' + this.id).then(resp=>{
              ...
              var result = md.render(resp.data.data.content)
