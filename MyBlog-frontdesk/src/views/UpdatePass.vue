		...
		methods: {
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
console.log("21e",this.passForm.currentPass)
						const _this = this
						this.$axios.put('/user/updatePass?currentPass='+this.passForm.currentPass+'&password='+this.passForm.password).then(res => {
							this.$message.success(res.data.msg);
              localStorage.removeItem("token")//删除jwt
