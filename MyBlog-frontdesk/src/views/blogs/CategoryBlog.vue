<template>
  <div>
    ...
    <div style="margin-top: 20px;">
      <el-pagination
       ...
       <el-form status-icon  label-width="120px">
         <el-form-item label="分类专栏名">
           <el-input v-model="add_type.name" placeholder="请输入要新建的分类专栏名"
           :rules="{
      ...
      <el-form status-icon  label-width="120px">
        <el-form-item label="分类专栏名">
          <el-input v-model="edit_type.name"
          :rules="{
            required: true, message: '文章类型不能为空', trigger: 'blur'
            }"></el-input>
        </el-form-item>
...
export default {
  ...
  methods:{
    ...
    initType(){
      const _this = this
      this.$axios.get('/type/getTypeByPage?current=' + this.currentPage + '&size=' + this.pagesize).then(resp=>{
        console.log(resp.data)
        _this.typeData = resp.data.data.records
    ...
    handleEdit(index, row) {
      ...
      _this.edit_type.id=row.id
      _this.edit_type.name=row.name
    },
    //更新分类
    updateType(){
      const _this = this
      this.$axios.put('/type/updateType',this.edit_type).then(resp=>{
        if(resp){
          this.initType()
    ...
    handleDelete(index, row) {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          this.$axios.delete('/type/deleteTypeById?id=' + row.id).then(resp=>{
            if(resp){
              this.initType()
            }
            this.reload() // 刷新当前页面
    ...
    addNewType(){
      const _this = this
      this.$axios.post('/type/addType',this.add_type.name).then(resp=>{
        console.log(resp)
        _this.add_type.name =''
