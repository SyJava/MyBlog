      ...
      <div>
          ...
          <el-table-column  label="操作"  fixed="right" width="220">
            <template slot-scope="scope">
              ...
              <!--携带邮箱号跳转到发送邮箱页面-->
              <el-button size="mini">
                <router-link :to="{path:'/email',query: {email: scope.row.email}}">邮件</router-link>
              </el-button>
              <el-button
                size="mini"
                type="danger"
...
export default {
  ...
  methods:{
    ...
    initComment(){
      const _this = this
      this.$axios.get('/comment/getCommentByPage?current=' + this.currentPage + '&size=' + this.pagesize).then(resp=>{
        console.log(resp)
        _this.commentData = resp.data.data.records
    ...
    handleLook(index, row){
      ...
      //如果评论不来自留言板，说明来自博客，则获取博客名称
      if (row.blogId!=-1) {
        this.$axios.get('/blog/getByBlogId?id=' + row.blogId).then(resp => {
          _this.BackComment.blogTitle = resp.data.data.title
        })
      }else {
        _this.BackComment.blogTitle = '来自留言板'
      }
      ...
      _this.subComment.parentCommentId = row.id
    ...
    submitBackComment(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const _this = this
          this.$axios.post('/comment/saveComments',this.subComment).then(resp=>{
            console.log(resp)
            _this.initComment()
    ...
    handleDelete(index, row) {
      ...
      this.$confirm('此操作将永久删除该评论, 是否继续?', '提示', {
          ...
          this.$axios.delete('/comment/deleteCommentById?id=' + row.id).then(resp=>{
            if(resp){
              this.initComment()
