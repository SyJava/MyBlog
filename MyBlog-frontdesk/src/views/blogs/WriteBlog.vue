    ...
    <div class="m_content">
      <el-form ref="editForm" status-icon :model="editForm" :rules="rules1"  label-width="80px">
        ...
        <el-form-item label="首页图片" prop="firstPicture">
          <el-input v-model="editForm.firstPicture"></el-input>
        </el-form-item>
        <el-form-item label="内容" prop="content">
          <mavon-editor :ishljs = "true" @imgAdd="$imgAdd" @imgDel="$imgDel" ref=md v-model="editForm.content"/>
        </el-form-item>
        <el-form-item style="margin: auto;">
          <el-button type="info" @click="temporarySave('editForm')">暂时保存</el-button>
...
<!--           <el-input v-model="oldtags" placeholder="请选择标签，多个标签请用逗号隔开" width="80%"></el-input>-->
        ...
        <el-form-item label="文章类型" prop="isOriginal" :label-width="formLabelWidth"
            :rules="{
              required: true, message: '文章类型不能为空', trigger: 'blur'
            }">
          <el-select v-model="editForm.isOriginal" placeholder="请选择文章类型">
            <el-option label="原创" value=true></el-option>
            <el-option label="转载" value=false></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="发布形式" prop="isPublished" :label-width="formLabelWidth"
            :rules="{
              required: true, message: '发布形式不能为空', trigger: 'blur'
            }">
          <el-select v-model="editForm.isPublished" placeholder="请选择发布形式">
...
<script>
  export default {
    ...
    data() {
      return {
        editForm: {  //博客文章表单
          ...
          tags: [
            // 添加默认标签为用户姓名
            {tagName:JSON.parse(localStorage.getItem("user")).username},
          ],
        },
        editForm2: {  //用来校验的表单
          typeId: null,
    ...
    methods: {
      ...
      addNewType(){
        const _this = this
        this.$axios.post('/type/addType',this.type.name).then(resp=>{
          if(resp){
            _this.type.name = ''
            this.initType()
            _this.dialog2 = false
      ...
      submitBlog(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            //发布博客
            this.$axios.post('/blog/savaBlog',this.editForm).then(resp=>{
              if(resp){
                _this.dialogFormVisible = false
                this.$message.success(resp.data.msg)
                _this.$router.replace('/admin/allblogs')
              }
            })
          } else {
            return false;
      ...
      temporarySave(formName){
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            //发布博客
            this.$axios.post('/blog/temporarySave',this.editForm).then(resp=>{
              if(resp){
                console.log(resp.data.data)
                _this.editForm.blogId=resp.data.data//保存或修改成功后，返回博客id，以免多次保存相同草稿
